...
<?php
   ...
   $query = "SELECT id, nombre, descripcion, codigo, item
            FROM ".dbname.".version
            WHERE version.id = $_GET[id]";
   $res = mysql_query($query) or die(mysql_error());
  ...
  if(isset($_POST['enviar'])) {
	...
	if ($old[nombre] != $nombre){
		 $query = "UPDATE ".dbname.".version SET nombre = '$nombre' WHERE id = $old[id]";
		 $res = mysql_query($query) or die(mysql_error());
	}
	if ($old[descripcion] != $descripcion){
		$query = "UPDATE ".dbname.".version SET descripcion = '$descripcion' WHERE id = $old[id]";
		$res = mysql_query($query) or die(mysql_error());
	}
	if ($old[codigo] != $estado_item){
		$query = "UPDATE ".dbname.".version SET codigo = '$estado_item' WHERE id = $old[id]";
		$res = mysql_query($query) or die(mysql_error());
	}
     ...
     // Formulario
	  ...
	  $nombre = str_replace('<br>', '\\n', $old[nombre]);
      echo "   <tr><td>Nombre(*) </td><td> <input type='text' name='nombre' size='30' maxlength='30' value='$nombre'></td></tr>";
	  $descripcion = str_replace('<br>', '\\n', $old[descripcion]);
      echo "   <tr><td>Descripción</td><td> <textarea rows='5' name='descripcion' cols='28'>$descripcion</textarea> </td></tr>";
      ...
      while ($row=mysql_fetch_array($res)){
		  if ($old[codigo] == $row[nombre]){
            echo "      <option selected value='$row[nombre]'>$row[nombre]</option></p>";
         } else {
